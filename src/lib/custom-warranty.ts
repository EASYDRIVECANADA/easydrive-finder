...
// WarrantyPlan so all downstream surfaces (warranty page, checkout, dealer
// configuration) work without per-plan branching.
//
// COST values live on the plan rows themselves (just like brochure plans).
// RETAIL is computed via the existing dealer-config override grid + markup.
...
type Store = {
  /** Custom plan list. Each plan has a slug starting with "custom-" so it never
   * collides with brochure slugs. */
  plans: WarrantyPlan[];
};
...
// Cached snapshot for stable useSyncExternalStore references.
